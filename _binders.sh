...
function bind_kbd_usb {
    ...
    OPTS="$OPTS -usb"
    ## Logitech G603
    OPTS="$OPTS -device usb-host,vendorid=0x046d,productid=0xc53d"
    ## Logitech G613
    OPTS="$OPTS -device usb-host,vendorid=0x046d,productid=0xc539"
    ## 4-port Hub
#    OPTS="$OPTS -device usb-host,vendorid=0x05e3,productid=0x0610"
}
...
function mount_iso_boot {
    ## Adding boot media
    OPTS="$OPTS -drive file=$VMs/isos/$1.iso,id=isocd,format=raw,if=none"
    OPTS="$OPTS -device scsi-cd,drive=isocd"
}

function mount_img_disk {
    ## Adding Win10 image
    OPTS="$OPTS -drive file=$VMs/imgs/$1.img,id=disk,format=qcow2,if=none,cache=writeback"
